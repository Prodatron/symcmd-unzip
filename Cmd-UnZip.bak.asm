...
uzifil  call uzidis             ;remove subdirectories
        ...
        scf
        ret
uzifil1 ld a,(uzitstflg)
        or a
        jr nz,uzifil2
        call uzishr             ;shorten filename
        jr z,uzifil2
        ;...                    ;test, if file already exists ##!!##
        ld hl,txtexfln4:call SyShell_STROUT0:ld hl,uzishrnam:call SyShell_STROUT0       ;as ...
uzifil2 ld hl,txtexfln5:call SyShell_STROUT0
        ld hl,uzishrnam
        or a
        ret
...
uzisub3 pop de
        ...
        ldir
        ;...                    ;test, if file already exists ##!!##
        ld hl,txtexfln4:call SyShell_STROUT0:ld hl,uzifilbuf:call SyShell_STROUT0       ;as ...
uzisub4 ld hl,txtexfln5:call SyShell_STROUT0
        ld hl,uzifilbuf
        or a
        ret
...
uziexf6 ld a,(hl)
        ...
        ld a,(uzitstflg)
        or a
        call z,outopn           ;create destination file
        ld a,6
        jp c,prgendf
uziexfb call outini
...
txterr006   db "Error while creating file",13,10,0
...
outfilnam   dw 0        ;output filename pointer
outfilnul   db 0
